...
import os
'''Read Files'''
os.chdir('/Users/allen/desktop/case_studies/dsi-recommender-case-study/data/')
...
for x in xrange(jokes_arr.shape[0]):
    lowers = str(jokes_arr[x]).lower().strip()
...
tfidfvect = TfidfVectorizer(stop_words='english', max_features=None)
tfidf_fit = tfidfvect.fit(jokes_lem)
tfidf_vectorized = tfidfvect.transform(jokes_lem)
